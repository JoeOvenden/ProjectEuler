...
class Hatched_Triangle:
    ...
    def get_solutions(self):
        for line in self.zero_lines:
            ...
            for root in line.nodes:     
                ...
                # for every node to the right of the root node
                for i in range(root_pointer + 1, len(line.nodes)):
                    node_0 = line.nodes[i]
                    
                    # for every non zero line that comes off of node_0
                    for L in node_0.lines:
                        if L.angle != 0:

                            # for every node on the line L, is it connected to root?
                            # if yes a triangle has been found
                            for node_1 in L.nodes:
                                if node_1.id != node_0.id:
                                    for node_1_line in node_1.lines:
                                        if root.on_line(node_1_line):
                                            solution = [root, node_0, node_1]
                                            self.add_solution(solution)
    ...
    def add_solution(self, solution):
        ...
        if binary_search(SOLUTION_IDS, solution_id, 0, len(SOLUTION_IDS) - 1) is False:
